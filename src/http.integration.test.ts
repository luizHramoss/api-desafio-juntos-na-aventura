...
vi.mock("./prisma", () => ({
  prisma: {
    ...
    adventure: {
      findUnique: mocks.adventureFindUnique,
    },
    reservation: {
...
describe("HTTP", () => {
  beforeEach(() => {
    ...
    mocks.pricingFindFirst.mockResolvedValue({
  ...
  it("POST /reservations 201", async () => {
    ...
    expect(res.body.share_url).toMatch(
      /\/join\/token-publico-123$/
    );
  });
  ...
  it("GET /public/:token mensagem de grupo confirmado", async () => {
    ...
    expect(res.body.message).toMatch(/Saída confirmada/i);
  });
